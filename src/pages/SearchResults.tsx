...
export default function SearchResults() {
  ...
  const [advancedFilters, setAdvancedFilters] = useState({
    ...
    homeType: ['house', 'condo', 'townhouse', 'apartment', 'land', 'commercial', 'other'] // More inclusive types
  });
  ...
  const parseRangeInput = (input: string, min: number, max: number): [number, number] => {
  ...
  const filteredProperties = properties.filter(property => {
    ...
    if (properties.length > 0) {
      console.log(`Filtering ${properties.length} total properties with filters:`, {
    ...
    if (properties.length > 0 && properties.length <= 5) {
      ...
      console.log('Property price:', properties[0].price, 'Type:', typeof properties[0].price);
      console.log('Property beds:', properties[0].beds, 'Type:', typeof properties[0].beds);
      console.log('Property baths:', properties[0].baths, 'Type:', typeof properties[0].baths);
    ...
    // Price filter
    if (property.price < advancedFilters.priceRange[0] || property.price > advancedFilters.priceRange[1]) {
      console.log(`Property ${property.id} filtered out by price: ${property.price} not in range [${advancedFilters.priceRange[0]}, ${advancedFilters.priceRange[1]}]`);
      return false;
    } else {
      console.log(`Property ${property.id} price ${property.price} is within range [${advancedFilters.priceRange[0]}, ${advancedFilters.priceRange[1]}]`);
    }

    // Bedrooms filter
    if (property.beds < advancedFilters.bedrooms[0] || property.beds > advancedFilters.bedrooms[1]) {
      console.log(`Property ${property.id} filtered out by beds: ${property.beds} not in range [${advancedFilters.bedrooms[0]}, ${advancedFilters.bedrooms[1]}]`);
      return false;
    }

    // Bathrooms filter
    if (property.baths < advancedFilters.bathrooms[0] || property.baths > advancedFilters.bathrooms[1]) {
      console.log(`Property ${property.id} filtered out by baths: ${property.baths} not in range [${advancedFilters.bathrooms[0]}, ${advancedFilters.bathrooms[1]}]`);
      return false;
    }

    // Home type filter
    if (!advancedFilters.homeType.includes(property.type)) {
      console.log(`Property ${property.id} filtered out by type: ${property.type} not in allowed types [${advancedFilters.homeType.join(', ')}]`);
      return false;
    }
  ...
  useEffect(() => {
    ...
    const propertyType = searchParams.get('propertyType');
      ...
      const hasCustomFilters =
        ...
        (propertyType && propertyType !== 'house,condo,townhouse,apartment,land,commercial,other');
    ...
    setAdvancedFilters(prev => ({
      ...prev,
       ...
       homeType: propertyType ? propertyType.split(',') : ['house', 'condo', 'townhouse', 'apartment', 'land', 'commercial', 'other']
    }));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6">
             ...
             filtersOpen={isFiltersOpen || (areFiltersEnabled && areFiltersActuallyApplied)}
           />
        </div>
               ...
               <div>
                 ...
                 {areFiltersEnabled && (
                   <p className="text-sm text-blue-600 mt-1">
                     Currently filtering {filteredProperties.length} of {properties.length} properties
                   </p>
                 )}
               </div>
               <div className="flex gap-2">
                     ...
                     onClick={() => {
                       setAdvancedFilters({
                         ...
                         bedrooms: [0, 10],
                         bathrooms: [0, 10],
                         homeType: ['house', 'condo', 'townhouse', 'apartment', 'land', 'commercial', 'other']
                       });
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className={!areFiltersEnabled ? "opacity-50" : ""}>
                ...
                <div className="px-2">
                      ...
                      onValueChange={(value) => setAdvancedFilters(prev => ({ ...prev, priceRange: value as [number, number] }))}
                      max={25000000}
                      min={0}
                      step={100000}
                      className="w-full"
                  ...
                  <div className="flex justify-between text-xs text-gray-600 mt-1">
                    <span className="font-medium text-blue-600">{formatPriceDisplay(advancedFilters.priceRange[0])}</span>
                    <span className="font-medium text-blue-600">{formatPriceDisplay(advancedFilters.priceRange[1])}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={!areFiltersEnabled ? "opacity-50" : ""}>
                ...
                <div className="px-2">
                     ...
                     value={bedroomInput}
                        ...
                        setAdvancedFilters(prev => ({ ...prev, bedrooms: parseRangeInput(e.target.value, 0, 10) }));
                      }}
              ...
              <div className={!areFiltersEnabled ? "opacity-50" : ""}>
                ...
                <div className="px-2">
                     ...
                     value={bathroomInput}
                        ...
                        setAdvancedFilters(prev => ({ ...prev, bathrooms: parseRangeInput(e.target.value, 0, 10) }));
                      }}
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => {
                        if (checked) {
                          ...
                          setAdvancedFilters(prev => ({
                            ...prev,
                            homeType: prev.homeType.filter(t => t !== type.value)
                          }));
                        }
                      }}
                      disabled={!areFiltersEnabled}
                    />
                    <label htmlFor={`filter-${type.value}`} className="text-sm text-gray-700">
                      {type.label}
                    </label>
                  </div>
                ))}
          ...
          <div className="text-center py-12">
            <div className="text-gray-400 mb-4">
              <Home size={64} className="mx-auto" />
                        ...
                        <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {areFiltersActuallyApplied ? 'No properties match your filters' : 'No properties found'}
            </h3>
            <p className="text-gray-600">
              {areFiltersActuallyApplied
